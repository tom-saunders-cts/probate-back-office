...
const scenarioName = 'Solicitor - Apply Grant of Probate - SolNotApply - CTC NoneOfThese -> CTC Partners PowerRes';
Scenario(scenarioName, async function ({I}) {
    ...
    {
        await I.fillField({css: '#primaryApplicantForenames'}, firstExecFN);
        await I.fillField({css: '#primaryApplicantSurname'}, firstExecFN);
        await I.click({css: '#primaryApplicantIsApplying_Yes'});

        // Manually enter address
        await I.click('.manual-link');
        await I.fillField('#primaryApplicantAddress__detailAddressLine1', address.line1);
        await I.fillField('#primaryApplicantAddress__detailPostTown', address.town);
        await I.fillField('#primaryApplicantAddress__detailPostCode', address.postcode);
        await I.fillField('#primaryApplicantAddress__detailCountry', address.country);

        await I.click({css: '#otherExecutorExists_Yes'});

        await I.click('#solsAdditionalExecutorList > div > button');
        await I.fillField({css: '#solsAdditionalExecutorList_0_additionalExecForenames'}, secondExecFN);
        await I.fillField({css: '#solsAdditionalExecutorList_0_additionalExecLastname'}, secondExecLN);
        await I.click({css: '#solsAdditionalExecutorList_0_additionalExecNameOnWill_No'});
        await I.click({css: '#solsAdditionalExecutorList_0_additionalApplying_No'});
        await I.click({css: '#solsAdditionalExecutorList_0_additionalExecReasonNotApplying-Renunciation'});
